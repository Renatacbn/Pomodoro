...
function iniciar(){
    if(acao.value == 0){
        ...
        document.getElementById('erro_pausa').innerHTML= "Adicione a sessão."
        sessao.focus()
    } else {
        lofi.play()
        pause.style.setProperty('display', 'block', 'important')
        ...
        localStorage.setItem('sessao', String(sessao.value))
        localStorage.setItem('sessao', String(sessao.value))
...
function momentoAcao(){
    ...
    let sessao_valor = localStorage.getItem('sessao')

    if( sessao_valor != '1' ){
        document.getElementById('title_sessao').innerHTML = sessao_valor + ' sessoes restantes'
    } else {
        document.getElementById('title_sessao').innerHTML = sessao_valor + ' sessao restante'
    }
...
function momentoPausa(){
    ...
    function segTimer(){
        ...
        if(segundos <= 0){
            if(min_pausa <= 0){
                ses = Number(localStorage.getItem('sessao'))
                ses = ses - 1
                localStorage.setItem('sessao', String(ses))
